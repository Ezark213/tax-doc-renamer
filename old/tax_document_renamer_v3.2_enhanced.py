...
class TaxDocumentRenamerGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("税務書類リネームシステム v3.2 機能強化版")
    ...
    def setup_file_tab(self):
        ...
        try:
            ...
            drop_frame = tk.Frame(self.file_tab, bg='lightgray', height=100)
            drop_frame.pack(fill='x', padx=10, pady=10)
            drop_frame.pack_propagate(False)
            
            drop_label = tk.Label(drop_frame, text="ファイル選択エリア\n（下のボタンでファイル選択）", 
    ...
    def setup_settings_tab(self):
        ...
        try:
            ...
            # 都道府県セット設定（5つ）
            prefecture_frame = ttk.LabelFrame(self.settings_tab, text="都道府県セット設定")
            prefecture_frame.pack(fill='x', padx=10, pady=10)
            
            self.prefecture_vars = []
            for i in range(5):
                frame = ttk.Frame(prefecture_frame)
                frame.pack(fill='x', pady=2)
                
                ttk.Label(frame, text=f"セット{i+1}:").pack(side='left')
                var = tk.StringVar()
                self.prefecture_vars.append(var)
                ttk.Entry(frame, textvariable=var, width=30).pack(side='left', padx=(5, 0))
                
            # デフォルト設定ボタン
            ttk.Button(prefecture_frame, text="デフォルト設定", command=self.set_default_prefectures).pack(pady=5)
    ...
    def setup_processors(self):
        ...
        try:
            ...
            self.debug_logger.log('INFO', "税務書類リネームシステム v3.2 機能強化版 起動")
            
        except Exception as e:
            messagebox.showerror("プロセッサー初期化エラー", f"システムの初期化に失敗しました:\n{e}")
    
    def set_default_prefectures(self):
        """デフォルト都道府県設定"""
        defaults = [
            "東京都",
            "愛知県",
        ...
        for i, default in enumerate(defaults):
            if i < len(self.prefecture_vars):
                self.prefecture_vars[i].set(default)
