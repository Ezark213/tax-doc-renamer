...
class TaxDocumentRenamerV5Final:
    """税務書類リネームシステム v5.0 最終修正版"""
    
    def __init__(self):
        """初期化"""
        self.root = tk.Tk()
        self.root.title("税務書類リネームシステム v5.0 最終版")
        ...
        # UI構築
        self._create_ui()
        self._log("税務書類リネームシステム v5.0 最終版を起動しました")
    ...
    def _create_ui(self):
        ...
        subtitle_label = ttk.Label(
            main_frame,
            text="セットベース連番システム + 都道府県選択 + 確実なファイル選択",
            font=('Arial', 10),
            foreground='blue'
        )
    ...
    def _create_main_tab(self):
        ...
        right_frame.pack(side='right', fill='y', padx=(2, 5), pady=5, ipadx=10)
        
        # === 左側: ファイル選択 ===
        # ファイル選択エリア（シンプル実装）
        file_select_frame = tk.Frame(left_frame, bg='#f0f0f0', relief='solid', bd=1)
    ...
    def _create_settings_tab(self):
        ...
        # 自治体セット設定
        sets_frame = ttk.LabelFrame(self.settings_frame, text="自治体セット設定")
        sets_frame.pack(fill='x', padx=10, pady=10)
        ...
        for set_num in range(1, 4):
            set_frame = ttk.Frame(sets_frame)
            set_frame.pack(fill='x', padx=5, pady=5)
            
            ttk.Label(set_frame, text=f"セット{set_num}:", width=8, font=('Arial', 10, 'bold')).pack(side='left')
            ...
            # 都道府県選択
            pref_var = tk.StringVar(value=default_pref)
            pref_combo = ttk.Combobox(set_frame, textvariable=pref_var, values=prefecture_options, width=12, state='readonly')
            pref_combo.pack(side='left', padx=(5, 0))
            
            # 市町村選択
            muni_var = tk.StringVar(value=default_muni)
            muni_combo = ttk.Combobox(set_frame, textvariable=muni_var, width=12, state='readonly')
            muni_combo.pack(side='left', padx=(5, 0))
            
            # 初期値設定
            if default_pref in municipality_options:
                muni_combo['values'] = municipality_options[default_pref]
    ...
    def _create_results_tab(self):
        ...
        for col in columns:
            ...
            width = 300 if col.endswith('ファイル名') else 120
    ...
    def _update_set_config(self, set_num, prefecture, municipality):
        ...
        # 都道府県コード計算
        pref_codes = {
            '東京都': 1001, '愛知県': 1011, '福岡県': 1021,
            '大阪府': 1031, '神奈川県': 1041, '北海道': 1051,
            '埼玉県': 1061, '千葉県': 1071, '静岡県': 1081,
            '広島県': 1091, '宮城県': 1101, '鹿児島県': 1111
        }
        
        # 市町村コード計算
        muni_codes = {
            '蒲郡市': 2001, '福岡市': 2011, '名古屋市': 2021,
            '大阪市': 2031, '横浜市': 2041, '札幌市': 2051
        }
        
        pref_code = pref_codes.get(prefecture, 1001)
