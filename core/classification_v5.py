...
class DocumentClassifierV5:
        ...
        return {
            ...
            "2003_受信通知": {
                ...
                "highest_priority_conditions": [
                    ...
                    AndCondition(["申告受付完了通知", "市町村申告書"], "all"),
                    AndCondition(["市町村申告書", "市役所", "申告受付完了通知"], "all"),
                    AndCondition(["市長", "市町村申告書", "受付完了通知"], "all"),
                    # 具体的な市町村名での判定
            ...
            "2004_納付情報": {
                ...
                "highest_priority_conditions": [
                    ...
                    AndCondition(["市役所", "納付情報", "市町村申告書"], "all"),
    ...
    def _apply_receipt_numbering_if_needed(self, classification_result: ClassificationResult, 
        ...
        """
        受信通知連番システム統合版 - ReceiptSequencer活用による動的連番計算
        ...
        # JobContext確認
        if not job_context:
            self._log_debug(f"[RECEIPT_SEQ] JobContext未設定のためスキップ: {base_code}")
            return classification_result
        
        # municipality_sets確認
        municipality_sets = getattr(job_context, 'current_municipality_sets', None)
        if not municipality_sets:
            self._log_debug(f"[RECEIPT_SEQ] municipality_sets未設定のためスキップ: {base_code}")
            return classification_result
            
        self._log(f"[RECEIPT_SEQ] 受信通知連番処理開始: {base_code}")
        
        try:
            # ReceiptSequencer統合による動的連番計算
            ...
            if is_pref_receipt(base_code):
                ...
                if prefecture:
                    ...
                    self._log(f"[RECEIPT_SEQ] 都道府県連番計算成功: {prefecture} → {final_code}")
                else:
                    self._log_debug(f"[RECEIPT_SEQ] 都道府県抽出失敗: {base_code}")
                ...
                if prefecture and city:
                    ...
                    self._log(f"[RECEIPT_SEQ] 市町村連番計算成功: {prefecture} {city} → {final_code}")
                else:
                    self._log_debug(f"[RECEIPT_SEQ] 市町村抽出失敗: pref={prefecture}, city={city}")
            ...
            if final_code and final_code != base_code.split('_')[0]:
                ...
                self._log(f"[RECEIPT_SEQ] 連番処理成功: {base_code} -> {new_document_type}")
                return new_result
            else:
                self._log(f"[RECEIPT_SEQ] 連番処理失敗またはスキップ: {base_code}")
                return classification_result
                
        except Exception as e:
            self._log(f"[RECEIPT_SEQ] 連番処理エラー: {e}")
            import traceback
            self._log_debug(f"[RECEIPT_SEQ] Traceback: {traceback.format_exc()}")
            return classification_result
    ...
    def _apply_municipality_numbering(self, document_type: str, 
        ...
        # 都道府県申告書（1001系統）
        if document_type == "1001_都道府県_都道府県申告書":
            if prefecture_code:
                ...
                final_code = f"{prefecture_code}_{prefecture_name}_都道府県申告書"
                
                # Use resolved code if different
                if resolved_code != document_type and "_" in resolved_code:
                    code_part = resolved_code.split("_")[0]
                    final_code = f"{code_part}_{prefecture_name}_都道府県申告書"
                
                self._log_debug(f"都道府県申告書連番適用: {document_type} → {final_code}")
                return final_code
        
        # 市町村申告書（2001系統）
        elif document_type.startswith("2") and document_type.endswith("_市町村_市町村申告書"):
            if municipality_code:
                ...
                final_code = f"{municipality_code}_{municipality_name}_市町村申告書"
        ...
        try:
            ...
            if is_prefecture_tax:
                ...
                if detected_set in pref_order_map:
                    ...
                    return f"{pref_code}_{resolved_pref}_都道府県申告書"
                ...
                if detected_set in city_order_map:
                    ...
                    return f"{city_code}_{resolved_pref}{resolved_city}_市町村申告書"
    ...
    def detect_page_doc_code(self, text: str, prefer_bundle: Optional[str] = None) -> Optional[str]:
        ...
        # 補助パターン: キーワード組み合わせ判定
        is_receipt = any(kw in text for kw in [
            "受信通知", "申告受付完了通知", "申告受付完了", "受付完了通知",
            # Phase 2: より柔軟な検出のためのキーワード追加
            "送信されたデータを受け付けました", "データを受け付けました",
            "メール詳細"  # 「受付番号」「利用者識別番号」は申告書にも印刷されるため除外
        ])
        is_payment = any(kw in text for kw in [
            "納付情報", "納付区分番号通知", "納付書", "納付情報発行結果",
            # Phase 2: より柔軟な検出のためのキーワード追加
            "納付区分番号", "納付情報発行", "メール詳細（納付"  # 括弧の一部でも検出
            # 「利用者識別番号」「受付番号」は申告書にも含まれるため削除
        ])
        ...
        # 都道府県キーワード（申告書にも含まれる税目名は除外）
        has_prefecture = any(kw in text for kw in [
            "都道府県", "県税事務所", "都税事務所",
            # 「法人事業税」「特別法人事業税」「法人県民税」等は申告書タイトルにも含まれるため削除
            "県民税", "道府県民税", "都民税",
            # 税目の詳細名
            "法人税割", "所得割", "付加価値割", "資本割",
            # 都道府県関連の一般的な表現
...
if __name__ == "__main__":
    ...
    def _apply_alternative_numbering_for_bundle_split(self, classification_result, base_code):
        """
        Bundle分割ファイル用の代替連番処理
        JobContextが未設定の場合でも基本的な連番付与を行う
        """
        self._log(f"[BUNDLE_ALTERNATIVE] Bundle分割ファイル代替連番処理開始: {base_code}")
        ...
        try:
            ...
            if base_code in ["1003_受信通知", "2003_受信通知"]:
                ...
                return ClassificationResult(
                    document_type=numbered_code,
                    confidence=classification_result.confidence,
                    matched_keywords=classification_result.matched_keywords + ["Bundle代替連番処理"],
                    classification_method="bundle_alternative_numbering",
                    debug_steps=classification_result.debug_steps + [f"Bundle代替連番: {base_code} → {numbered_code}"],
                    processing_log=self.processing_log.copy(),
                    original_filename=getattr(classification_result, 'original_filename', '')
                )
            
            # その他のBundle分割ファイルはそのまま返す
            self._log(f"[BUNDLE_ALTERNATIVE] 非対象書類のためそのまま返却: {base_code}")
            return classification_result
