#!/usr/bin/env python3
"""
税務書類リネームシステム v4.0 完全版
バグ修正・分割機能・優先度エンジン・東京都制御実装
"""
...
class KeywordPriorityEngine:
    ...
    def determine_document_type(self, text: str, filename: str, file_ext: str) -> Tuple[str, str]:
        ...
        for (keyword1, keyword2), pattern in self.and_conditions.items():
            if (keyword1 in text or keyword1 in filename) and (keyword2 in text or keyword2 in filename):
...
class TaxDocumentRenamerGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("税務書類リネームシステム v4.0 完全版")
    ...
    def setup_result_tab(self):
        ...
        try:
            ...
            # Treeview作成
            columns = ('元ファイル', '新ファイル名', '文書種別', 'YYMM', 'ステータス')
    ...
    def setup_processors(self):
        ...
        try:
            ...
            self.debug_logger.log('INFO', "税務書類リネームシステム v4.0 完全版 起動")
            ...
            for file_path in self.files_to_process:
                ...
                for result in results:
                    ...
                    if result['success']:
                        # ファイルリネーム実行
                        try:
                            source_path = Path(result.get('split_file', file_path))
    ...
    def update_result_display(self, results):
        ...
        try:
            ...
            for result in results:
                ...
                self.result_tree.insert('', 'end', values=(original_file, new_name, doc_type, yymm, status))
        ...
        try:
            ...
            if file_path:
                with open(file_path, 'w', encoding='utf-8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(['元ファイル', '新ファイル名', '文書種別', 'YYMM', 'ステータス', 'エラー詳細'])
                    
                    for result in self.result_data:
                        writer.writerow([
                            os.path.basename(result['original_file']),
