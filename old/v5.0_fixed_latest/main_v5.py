...
from core.classification_v5_fixed import DocumentClassifierV5Fixed  # v5.0修正版エンジンを使用
...
class TaxDocumentRenamerV5:
    ...
    def __init__(self):
        """初期化"""
        self.root = tk.Tk()
        self.root.title("税務書類リネームシステム v5.0 (AND条件対応版)")
        ...
        self.classifier_v5 = DocumentClassifierV5Fixed(debug_mode=True, log_callback=self._log)
    ...
    def _create_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, 
            text="税務書類リネームシステム v5.0 (AND条件対応版)", 
            font=('Arial', 16, 'bold')
        )
        ...
        info_label = ttk.Label(
            main_frame,
            text="✨ v5.0新機能: AND条件判定・受信通知/納付情報の高精度認識",
            font=('Arial', 10),
            foreground='blue'
        )
    ...
    def _process_regular_pdf_v5(self, file_path: str, output_folder: str):
        ...
        document_type, alerts = self.classifier_v5.classify_document_v5_fixed(text, filename)
        
        # 結果オブジェクトを作成（互換性のため）
        class ClassificationResult:
            def __init__(self, document_type, alerts):
                self.document_type = document_type
                self.alerts = alerts
                self.confidence = 1.0 if document_type != "9999_未分類" else 0.0
                self.classification_method = "v5.0_fixed_engine"
                self.matched_keywords = []
                # alertsから一致キーワードを抽出
                for alert in alerts:
                    if hasattr(alert, 'suggestions') and alert.suggestions:
                        self.matched_keywords.extend(alert.suggestions)
                
        classification_result = ClassificationResult(document_type, alerts)
        
        # 分類詳細ログを出力（v5.0修正版）
        self._log(f"v5.0分類結果詳細:")
        ...
        for i, alert in enumerate(alerts):
            self._log(f"  - アラート{i+1}: {alert.alert_type} - {alert.message} (信頼度: {alert.confidence:.2f})")
            if alert.suggestions:
                self._log(f"    └ キーワード: {alert.suggestions}")
