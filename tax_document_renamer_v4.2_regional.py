#!/usr/bin/env python3
"""
税務書類リネームシステム v4.2 地域対応版
分割制御・地域判定エンジン・法人税判定優先度修正実装
"""
...
class RegionalDetectionEngine:
    ...
    def __init__(self, debug_logger):
        ...
        self.setup_regional_patterns()
        
    def setup_regional_patterns(self):
        """地域判定パターン設定"""
        ...
        self.prefecture_codes = {
            1: "1001", 2: "1011", 3: "1021", 4: "1031", 5: "1041"
        }
        
        self.city_codes = {
            1: "2001", 2: "2011", 3: "2021", 4: "2031", 5: "2041"
        }
        
    def detect_prefecture_from_text(self, text: str) -> Optional[str]:
        """テキストから都道府県を検出"""
        for prefecture, patterns in self.prefecture_patterns.items():
            for pattern in patterns:
                if re.search(pattern, text):
                    self.debug_logger.log('INFO', f"都道府県検出: {prefecture} (パターン: {pattern})")
                    return prefecture
        return None
    ...
    def detect_city_from_text(self, text: str) -> Optional[str]:
        ...
        for city, patterns in self.city_patterns.items():
            for pattern in patterns:
                if re.search(pattern, text):
                    self.debug_logger.log('INFO', f"市町村検出: {city} (パターン: {pattern})")
                    return city
        return None
        
    def get_prefecture_set_number(self, prefecture_selections: List[str], detected_prefecture: str) -> Optional[int]:
        """検出された都道府県のセット番号を取得"""
        for i, selection in enumerate(prefecture_selections):
            if selection == detected_prefecture:
                return i + 1  # 1ベースのセット番号
        return None
        
    def get_city_set_info(self, prefecture_selections: List[str], city_selections: List[str], detected_city: str) -> Optional[Tuple[int, str]]:
        """検出された市町村のセット番号と都道府県を取得"""
        for i, city_selection in enumerate(city_selections):
            if detected_city in city_selection:
                prefecture = prefecture_selections[i]
                if prefecture and prefecture != "東京都":  # 東京都は市町村申告なし
                    return i + 1, prefecture  # セット番号と都道府県
        return None
        
    def generate_regional_filename(self, document_type: str, prefecture: str, city: str, set_number: int, yymm: str) -> Optional[str]:
        """地域別ファイル名生成"""
        
        if document_type == "都道府県申告":
            code = self.prefecture_codes.get(set_number)
            if code:
                return f"{code}_{prefecture}_法人都道府県民税・事業税・特別法人事業税_{yymm}.pdf"
                
        elif document_type == "市町村申告" and prefecture != "東京都":
            code = self.city_codes.get(set_number)
            if code:
...
class PDFSplitter:
    ...
    def __init__(self, debug_logger):
        self.debug_logger = debug_logger
        self.setup_no_split_patterns()
        
    def setup_no_split_patterns(self):
        """分割除外パターン設定"""
        # 修正：分割不要書類パターン
    ...
    def should_split_pdf(self, pdf_path: str, ocr_text: str) -> bool:
        ...
        for pattern in self.no_split_patterns:
            if pattern in ocr_text or pattern in filename:
                self.debug_logger.log('INFO', f"分割除外: '{pattern}' パターンにより分割しません")
                return False
    ...
    def detect_split_points(self, pdf_path: str) -> List[int]:
        ...
        try:
            ...
            if not self.should_split_pdf(pdf_path, first_page_text):
                doc.close()
                return []  # 分割しない
            ...
            for page_num in range(len(doc)):
                ...
                # 空白ページ判定（テキスト50文字未満は除外）
                if len(text) >= 50:
                    split_points.append(page_num)
                    self.debug_logger.log('DEBUG', f"有効ページ検出: ページ{page_num + 1} ({len(text)}文字)")
                else:
                    self.debug_logger.log('DEBUG', f"空白ページ除外: ページ{page_num + 1} ({len(text)}文字)")
                    
            doc.close()
            
            self.debug_logger.log('INFO', f"有効ページ: {[p+1 for p in split_points]}")
            return split_points
    ...
    def split_pdf(self, pdf_path: str, split_points: List[int], output_dir: str) -> List[str]:
        ...
        try:
            ...
            # 各ページを個別ファイルとして分割
            for i, page_num in enumerate(split_points):
                # 新しいPDF作成（1ページのみ）
                new_doc = fitz.open()
                new_doc.insert_pdf(doc, from_page=page_num, to_page=page_num)
                
                # 一時ファイルとして保存
                temp_path = os.path.join(output_dir, f"{base_name}_page_{page_num+1}.pdf")
                new_doc.save(temp_path)
                new_doc.close()
                
                split_files.append(temp_path)
                self.debug_logger.log('INFO', f"分割ファイル作成: {temp_path} (ページ{page_num+1})")
...
class DocumentProcessor:
    ...
    def update_regional_settings(self, prefecture_selections: List[str], city_selections: List[str]):
        """地域設定更新"""
        self.prefecture_selections = prefecture_selections
        self.city_selections = city_selections
        
        # 優先度エンジンも更新
        self.priority_engine = KeywordPriorityEngine(
            self.debug_logger, self.regional_engine,
            self.prefecture_selections, self.city_selections
        )
            ...
            if file_ext == '.pdf':
                ...
                if len(split_points) > 1:
                    ...
                    for i, split_file in enumerate(split_files):
                        ...
                        result['split_info'] = f"ページ分割{i+1}/{len(split_files)}"  # 分割情報追加
...
class TaxDocumentRenamerGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("税務書類リネームシステム v4.2 地域対応版")
    ...
    def setup_file_tab(self):
        ...
        try:
            ...
            # 都道府県・市町村入力エリア
            prefecture_frame = ttk.LabelFrame(self.file_tab, text="都道府県・市町村設定（地域判定用）")
    ...
    def setup_result_tab(self):
        ...
        try:
            ...
            # カラム設定
            column_widths = {'元ファイル': 200, '分割情報': 120, '新ファイル名': 250, '文書種別': 150, 'YYMM': 80, 'ステータス': 150}
            for col in columns:
    ...
    def setup_processors(self):
        ...
        try:
            ...
            self.debug_logger.log('INFO', "税務書類リネームシステム v4.2 地域対応版 起動")
    ...
    def process_files(self):
        ...
        try:
            ...
            self.processor.update_regional_settings(prefecture_selections, city_selections)
            ...
            for file_path in self.files_to_process:
                ...
                for result in results:
                    ...
                    if result['success']:
                        # ファイルリネーム実行
                        try:
                            source_path = Path(result['original_file'])  # 分割ファイルのパスを使用
