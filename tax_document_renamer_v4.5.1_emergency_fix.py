#!/usr/bin/env python3
"""
税務書類リネームシステム v4.5.1 緊急バグ修正版
v4.2をベース、緊急バグ修正要件定義に従った最高優先度判定の実装
"""
...
class EmergencyClassifier:
    ...
    def __init__(self, debug_logger):
        ...
        self.setup_emergency_patterns()
    
    def setup_emergency_patterns(self):
        """緊急修正：最高優先度パターン設定"""
        
        # 緊急修正：消費税申告書の絶対優先（地域名より優先）
    ...
    def emergency_highest_priority_check(self, ocr_text: str, filename: str) -> Optional[str]:
        ...
        self.debug_logger.log('INFO', "=== 緊急修正：最高優先度判定開始 ===")
        ...
        self.debug_logger.log('DEBUG', "緊急修正：最高優先度に該当なし")
        return None
...
class MunicipalReceiptDetector:
    ...
    def __init__(self, debug_logger):
        self.debug_logger = debug_logger
        self.setup_municipal_patterns()
        
    def setup_municipal_patterns(self):
        """市町村受信通知パターン設定"""
    ...
    def detect_municipal_receipt_notice(self, ocr_text: str) -> Optional[str]:
        """市町村受信通知の判定"""
        
        self.debug_logger.log('INFO', "市町村受信通知判定開始")
        ...
        if has_municipal_keywords:
            ...
            for pattern in self.municipal_patterns:
                ...
                if match:
                    ...
                    self.debug_logger.log('INFO', f"市町村受信通知判定成功: {municipality} → 2003")
                    return "2003_受信通知_{yymm}.pdf"  # 連番は後で設定
        
        self.debug_logger.log('DEBUG', "市町村受信通知判定：該当なし")
        return None
...
class AccuratePrefectureDetector:
    ...
    def __init__(self, debug_logger):
        self.debug_logger = debug_logger
        self.setup_accurate_patterns()
        
    def setup_accurate_patterns(self):
        """正確な都道府県名検出パターン"""
    ...
    def accurate_prefecture_detection(self, ocr_text: str) -> Optional[str]:
        ...
        self.debug_logger.log('INFO', "正確な都道府県名検出開始")
        
        for pattern in self.prefecture_office_patterns:
            match = re.search(pattern, ocr_text)
            if match:
                prefecture = match.group(1)
                # 確実性チェック（事務所名の直前にある都道府県名のみ）
                if prefecture in ['東京都', '北海道'] or prefecture.endswith(('県', '府')):
                    self.debug_logger.log('INFO', f"都道府県名検出成功（修正版）: {prefecture}")
                    return prefecture
        
        self.debug_logger.log('WARNING', "都道府県名検出失敗（修正版）")
        return None
...
class CorrectSequenceGenerator:
    ...
    def generate_correct_sequence_number(self, document_type: str, order: int) -> Optional[int]:
        """要件定義準拠の連番生成"""
        ...
        if document_type in base_numbers:
            ...
            if document_type == 'municipal_receipt':
                # 市町村受信通知：2003, 2013, 2023, 2033, 2043
                result = base + (order * 10)
            else:
                # 都道府県・市町村：1001→1011, 2001→2011
                result = base + (order * 10)
                
            self.debug_logger.log('INFO', f"連番生成（修正版）: {document_type} order={order} → {result}")
            return result
        
        return None

class RegionalDetectionEngine:
    """地域判定エンジン（v4.2継承・精度修正版）"""
    ...
    def detect_prefecture_from_text(self, text: str) -> Optional[str]:
        ...
        # まず正確な検出を試行
        accurate_result = self.prefecture_detector.accurate_prefecture_detection(text)
        if accurate_result:
        ...
        for prefecture in self.prefectures:
            if prefecture in text:
                self.debug_logger.log('DEBUG', f"都道府県検出（フォールバック）: {prefecture}")
                return prefecture
        return None
    ...
    def generate_regional_filename(self, doc_type: str, prefecture: str, 
        ...
        if doc_type == "都道府県申告":
            # 連番修正：1001, 1011, 1021...
            code = self.sequence_generator.generate_correct_sequence_number('prefecture', set_number - 1)
            return f"{code}_{prefecture}_法人都道府県民税・事業税・特別法人事業税_{yymm}.pdf"
        elif doc_type == "市町村申告":
            # 連番修正：2001, 2011, 2021...
            code = self.sequence_generator.generate_correct_sequence_number('municipality', set_number - 1)
            return f"{code}_{prefecture}{city}_法人市民税_{yymm}.pdf"
        return None

class EmergencyKeywordPriorityEngine:
    """緊急修正版：キーワード優先度エンジン（5001以降完全保護）"""
    ...
    def classify_document_with_emergency_fix(self, text: str, filename: str, file_ext: str, yymm: str) -> Tuple[str, str]:
        """緊急修正版：文書分類（最高優先度を最初に実行）"""
        
        self.debug_logger.log('INFO', f"緊急修正版分類開始: {filename}")
        ...
        # 【緊急修正】最高優先度判定（他の全判定より先に実行）
        emergency_result = self.emergency_classifier.emergency_highest_priority_check(text, filename)
        if emergency_result:
            final_result = emergency_result.replace('{yymm}', yymm)
            self.debug_logger.log('INFO', f"緊急修正：最高優先度適用 → {final_result}")
            return '最高優先度', final_result
        
        # 【緊急修正】市町村受信通知判定
        municipal_result = self.municipal_detector.detect_municipal_receipt_notice(text)
        if municipal_result:
            final_result = municipal_result.replace('{yymm}', yymm)
            self.debug_logger.log('INFO', f"緊急修正：市町村受信通知適用 → {final_result}")
            return '市町村受信通知', final_result
        
        # 【重要】5001以降の書類判定（完全保護）
        for keyword, pattern in self.protected_5000_patterns.items():
            if keyword in text or keyword in filename.lower():
                final_result = pattern.replace('{yymm}', yymm)
                self.debug_logger.log('INFO', f"5000番台判定（保護）: '{keyword}' → {final_result}")
                return keyword, final_result
        
        # 地域判定（地方税関連）
        regional_result = self._determine_regional_type(text, yymm)
        ...
        for keyword, pattern in self.receipt_payment_patterns.items():
            if re.search(keyword, text) or re.search(keyword, filename.lower()):
                final_result = pattern.replace('{yymm}', yymm)
                self.debug_logger.log('INFO', f"受信通知・納付情報判定: '{keyword}' → {final_result}")
                return keyword, final_result
        ...
        for keyword, pattern in self.high_priority_patterns.items():
            if keyword in text or keyword in filename.lower():
                final_result = pattern.replace('{yymm}', yymm)
                self.debug_logger.log('INFO', f"高優先度判定: '{keyword}' → {final_result}")
                return keyword, final_result
                
        self.debug_logger.log('WARNING', "文書種別判定不可")
        return '不明', f'未分類_{yymm}.pdf'
        
    def _determine_regional_type(self, text: str, yymm: str) -> Tuple[str, str]:
        """地域判定による文書種別決定（精度修正版）"""
        ...
        if detected_prefecture:
            ...
            if set_number:
                ...
                if filename:
                    self.debug_logger.log('INFO', f"都道府県申告判定（修正版）: {detected_prefecture} → {filename}")
...
class PDFProcessor:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                if len(text) >= 50:
                    ...
                    temp_filename = os.path.join(output_dir, f"temp_{page_num+1:03d}.pdf")
                    ...
                    self.debug_logger.log('DEBUG', f"空白ページ除外: ページ{page_num + 1} ({len(text)}文字)")
            
            doc.close()
            
            # 分割結果の確認
            if len(temp_files) <= 1:
                # 実質的に分割不要の場合は元ファイルを返す
    ...
    def rename_temp_files_to_final(self, temp_files: List[str], 
                                   priority_engine: EmergencyKeywordPriorityEngine,
                                   yymm_processor,
                                   user_yymm: str) -> List[Dict]:
        """Stage2: 一時ファイル→最終ファイル名変換（緊急修正版）"""
        results = []
        ...
        for temp_file in temp_files:
            try:
                ...
                # 緊急修正版文書種別判定
                doc_type, final_filename_or_pattern = priority_engine.classify_document_with_emergency_fix(
                    ocr_text, filename, '.pdf', yymm
                )
                
                if final_filename_or_pattern and not final_filename_or_pattern.endswith('.pdf'):
                    # パターンからファイル名生成
                    final_filename = final_filename_or_pattern
                elif final_filename_or_pattern:
                    final_filename = final_filename_or_pattern
                else:
                    # 判定失敗
                    final_filename = f"未分類_{yymm}.pdf"
                ...
                self.debug_logger.log('INFO', f"リネーム成功（緊急修正版）: {filename} → {final_filename}")
                    
            except Exception as e:
                self.debug_logger.log('ERROR', f"リネーム処理エラー（緊急修正版）: {e}")
                results.append({
                    'original_file': temp_file,
                    'new_file': temp_file,
                    'document_type': 'エラー',
...
class EmergencyProcessor:
    ...
    def __init__(self, debug_logger, prefecture_selections=None, city_selections=None):
        ...
        self.priority_engine = EmergencyKeywordPriorityEngine(
    ...
    def process_single_file(self, file_path: str, user_yymm: str = "") -> List[Dict]:
        ...
        try:
            self.debug_logger.log('INFO', f"=== 緊急修正版処理開始: {os.path.basename(file_path)} ===")
            ...
            self.debug_logger.log('ERROR', f"ファイル処理エラー（緊急修正版）: {e}")
            return [{
                'original_file': file_path,
                'new_file': file_path,
                'document_type': 'エラー',
    ...
    def process_csv_file(self, file_path: str, user_yymm: str = "") -> Dict:
        ...
        try:
            ...
            # 文書種別判定（緊急修正版）
            doc_type, final_filename_or_pattern = self.priority_engine.classify_document_with_emergency_fix(
                content, filename, '.csv', yymm
            )
            
            if final_filename_or_pattern and final_filename_or_pattern != '未分類.csv':
                # 最終ファイル名生成
                if '{yymm}' in final_filename_or_pattern:
                    final_filename = final_filename_or_pattern.replace('{yymm}', yymm)
                else:
                    final_filename = final_filename_or_pattern
            ...
            self.debug_logger.log('ERROR', f"CSV処理エラー（緊急修正版）: {e}")
            return {
                'original_file': file_path,
                'new_file': file_path,
                'document_type': 'エラー',
    ...
    def process_multiple_files(self, file_paths: List[str], user_yymm: str = "") -> List[Dict]:
        ...
        try:
            self.debug_logger.log('INFO', f"=== 緊急修正版複数ファイル処理開始: {len(file_paths)}件 ===")
            ...
            self.debug_logger.log('INFO', f"=== 緊急修正版複数ファイル処理完了: {len(all_results)}件処理 ===")
            return all_results
            
        except Exception as e:
            self.debug_logger.log('ERROR', f"複数ファイル処理エラー（緊急修正版）: {e}")
            return all_results

class TaxDocumentRenamerApp:
    """税務書類リネームアプリケーション v4.5.1 緊急バグ修正版"""
    
    def __init__(self, root):
        self.root = root
        self.root.title("税務書類リネームシステム v4.5.1 緊急バグ修正版")
        ...
        self.debug_logger.log('INFO', "税務書類リネームシステム v4.5.1 緊急バグ修正版 起動完了")
    ...
    def setup_main_tab(self):
        ...
        ttk.Button(process_frame, text="緊急修正版処理実行", command=self.start_processing).pack(side=tk.LEFT, padx=5)
        ttk.Button(process_frame, text="設定保存", command=self.save_settings).pack(side=tk.LEFT, padx=5)
        
        # ステータス
        self.status_var = tk.StringVar(value="v4.5.1 緊急修正版 準備完了")
    ...
    def clear_files(self):
        ...
        self.selected_files = []
        self.update_file_listbox()
        self.status_var.set("v4.5.1 緊急修正版 準備完了")
    ...
    def process_files(self):
        ...
        try:
            ...
            # 緊急修正版プロセッサー初期化
            self.processor = EmergencyProcessor(
                self.debug_logger, 
                prefecture_selections, 
                city_selections
            )
            ...
            self.root.after(0, lambda: self.status_var.set(f"緊急修正版処理完了: {len(results)}件"))
            
        except Exception as e:
            self.debug_logger.log('ERROR', f"緊急修正版処理エラー: {e}")
            self.root.after(0, lambda: self.status_var.set("緊急修正版処理エラー"))
